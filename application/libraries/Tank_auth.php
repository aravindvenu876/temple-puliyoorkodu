...
class Tank_auth {
     ...
     * @return	bool
     */
    function login($login, $password, $role, $remember, $login_by_username, $login_by_email) {
            ...
            if (!is_null($user = $this->ci->users->$get_user_func($login))) { // login ok
                ...
                if ($hasher->CheckPassword($password, $user->password)) {  // password ok
                    if ($user->banned == 1) {         // fail - banned
                        ...
                        if($this->ci->users->check_user_role($user->user_id,$role)){
                            $this->ci->session->set_userdata(array(
                                'user_id' => $user->user_id,
                                'database'  => 'default',
                                'fin_year'  => '2022 September To 2023 March',
                            ...
                            $this->error = array('role' => 'You dont have permission to login as this role');
                        }
                    }
    ...
    public function create_hashed_password($pwd) {
        $hasher = new PasswordHash(
                $this->ci->config->item('phpass_hash_strength', 'tank_auth'), $this->ci->config->item('phpass_hash_portable', 'tank_auth'));
