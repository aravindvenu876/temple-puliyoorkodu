...
class Role_model extends CI_Model {
    ...
    function get_all_roles($iDisplayStart, $iDisplayLength, $iSortCol_0, $iSortingCols, $sSearch, $sEcho){
        $sTable = 'user_roles';
        //* Array of database columns which should be read and sent back to DataTables. Use a space where
        //* you want to insert a non-database field (for example a counter or static image)
        $aColumns = array('id','role', 'status');
    ...
    function check_role_duplicate($role,$id=""){
        $this->db->select('*');
        $this->db->where('role',$role);
        if($id != ""){
            $this->db->where('id !=',$id);
        }
        $num = $this->db->get('user_roles')->num_rows();
        if($num == 0){
            return false;
        }else{
            return true;
