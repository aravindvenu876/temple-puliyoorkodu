...
class Api_model extends CI_Model {
    ...
    function get_devotee_by_family_name($name,$language){
        ...
        // $this->db->like('LOWER(name)=',strtolower($name),'after');
        $this->db->like('family_address',$name,'after');
        $this->db->limit(20);
        return $this->db->get('devotee_master')->result();
    }
    ...
    function add_receipt_detail($data){
        return $this->db->insert('receipt_details', $data);
    }
    ...
    function get_asset_receipt_details($id,$language){
        $this->db->select('receipt_details.*,DATE_FORMAT(date, "%d-%m-%Y") as date,asset_master_lang.asset_name as pooja,unit.notation,unit_lang.unit');
        $this->db->from('receipt_details');
        $this->db->join('asset_master_lang','asset_master_lang.asset_master_id=receipt_details.asset_master_id');
    ...
    function get_prasadam_receipt_details($id,$language){
        $this->db->select('receipt_details.*,DATE_FORMAT(date, "%d-%m-%Y") as date,item_master_lang.name as pooja');
        $this->db->from('receipt_details');
    ...
    function get_prasadam_receipt_details_from_pooja($id,$language){
        $this->db->select('receipt_details.*,DATE_FORMAT(date, "%d-%m-%Y") as date,item_master_lang.name as prasadam');
        $this->db->from('receipt_details');
        $this->db->join('item_master_lang','item_master_lang.item_master_id=receipt_details.item_master_id');
        $this->db->where('item_master_lang.lang_id',$language);
        $this->db->where('receipt_details.receipt_id',$id);
    ...
    function scheduled_pooja_date($id,$type,$language){
        $this->db->select('receipt_details.*,DATE_FORMAT(date, "%d-%m-%Y") as date,pooja_master_lang.pooja_name as pooja');
        $this->db->from('receipt_details');
        $this->db->join('pooja_master_lang','pooja_master_lang.pooja_master_id=receipt_details.pooja_master_id');
        $this->db->where('pooja_master_lang.lang_id',$language);
        $this->db->where('receipt_details.receipt_id',$id);
        $this->db->order_by('receipt_details.date',$type);
    ...
    function get_session_closing_amount($id){
        return $this->db->select('sum(receipt_amount) as closing_amount')->where('receipt_type !=','Nadavaravu')->where('receipt_status','ACTIVE')->where('session_id',$id)->get('receipt')->row_array();
    }

    function get_session_closing_amount_breakup($id){
        return $this->db->select('pay_type,sum(receipt_amount) as closing_amount')->where('receipt_type !=','Nadavaravu')->where('receipt_status','ACTIVE')->where('session_id',$id)->group_by('pay_type')->get('receipt')->result();
    }
    ...
    function get_current_session_receipt($session_id,$receipt_no=NULL){
        if($receipt_no == ""){
            return $this->db->select('*')->where('receipt_status','ACTIVE')->where('session_id',$session_id)->get('receipt')->result();
        }else{
            return $this->db->select('*')->where('receipt_status','ACTIVE')->where('receipt_no',$receipt_no)->get('receipt')->result();
        }
    }
    ...
    function get_counter_receipts($date,$receipt_no=NULL){
        if($receipt_no == ""){
            return $this->db->select('*')->where('receipt_status','ACTIVE')->where('receipt_date',$date)->get('receipt')->result();
        }else{
            return $this->db->select('*')->where('receipt_status','ACTIVE')->where('receipt_no',$receipt_no)->get('receipt')->result();
        }
    }

    function get_counter_receipts_by_pagination($date,$receipt_no=NULL,$value,$page){
        $start = (($page-1)*$value);
        $this->db->select('*,DATE_FORMAT(receipt_date, "%d-%m-%Y") as receipt_date');
        $this->db->where('receipt_status','ACTIVE');
        if($receipt_no == ""){
    ...
    function get_advance_receipt_details($id){
        $this->db->select('receipt_details.*,receipt.postal_check');
        $this->db->from('receipt_details');
        $this->db->join('receipt','receipt.id = receipt_details.receipt_id');
    ...
    function get_balithara_details_by_pagination($lang,$phone="",$date="",$value,$page){
        ...
        DATE_FORMAT(due_date, "%D %M %Y") as due_date,DATE_FORMAT(pay_date, "%M") as month,DATE_FORMAT(pay_date, "%Y") as year,balithara_master_lang.name as balithara_name');
        $this->db->from('balithara_auction_master');
        $this->db->join('balithara_auction_details','balithara_auction_details.master_id=balithara_auction_master.id');
        $this->db->join('balithara_master_lang','balithara_master_lang.balithara_id=balithara_auction_master.balithara_id');
        $this->db->where('balithara_auction_master.status','BOOKED');
    ...
    function get_draft_receipts_by_phone($phone,$date){
        // $this->db->distinct();
        $today = date('Y-m-d');
    ...
    function get_draft_receipts_by_phone_by_pagination($phone,$date,$value,$page){
        $start = (($page-1)*$value);
        // $this->db->distinct();
        $this->db->select('receipt.*,DATE_FORMAT(receipt.receipt_date, "%d-%m-%Y") as receipt_date');
        $this->db->from('receipt_details');
    ...
    function get_cancelled_receipts($date,$receipt_no=NULL){
        if($receipt_no == ""){
            return $this->db->select('*')->where('receipt_status','CANCELLED')->where('cancelled_on',$date)->get('receipt')->result();
        }else{
            return $this->db->select('*')->where('receipt_status','CANCELLED')->where('receipt_no',$receipt_no)->get('receipt')->result();
        }
    }
    ...
    function check_if_early_balithara_payment($mainId,$detailId){
        ...
        $count = $this->db->where('id <',$detailId)->get('balithara_auction_details')->num_rows();
        if($count == 0){
            return TRUE;
        }else{
            return FALSE;
