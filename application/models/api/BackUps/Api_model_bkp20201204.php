...
class Api_model extends CI_Model {
    ...
    function get_session_closing_amount($id){
        return $this->db->select('sum(receipt_amount) as closing_amount')->where('receipt_status','ACTIVE')->where('session_id',$id)->get('receipt')->row_array();
    }

    function get_session_closing_amount_breakup($id){
        return $this->db->select('pay_type,sum(receipt_amount) as closing_amount')->where('receipt_status','ACTIVE')->where('session_id',$id)->group_by('pay_type')->get('receipt')->result();
    }
    ...
    function add_receipt_detail_new($data){
        return $this->db->insert_batch('receipt_details', $data);
    }
