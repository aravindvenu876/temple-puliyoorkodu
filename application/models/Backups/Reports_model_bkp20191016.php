...
class Reports_model extends CI_Model {
    ...
    function get_cancel_report($filterData){
        // $this->db->select('receipt.receipt_no,receipt.receipt_type,receipt.receipt_status,receipt.receipt_date,
		// receipt.receipt_amount,users.name,counters.counter_no,pooja_master_lang.pooja_name as pooja');
		// $this->db->select('receipt.receipt_no,receipt.receipt_type,receipt.receipt_status,receipt.receipt_date,
        // receipt_details.amount as receipt_amount,users.name,counters.counter_no,pooja_master_lang.pooja_name as pooja');
        // $this->db->from('receipt');
        // $this->db->join('users','users.id=receipt.user_id');
	...
	function get_mattuvarumanam_report($filterData){
		$this->db->select('receipt.receipt_date as date,receipt.receipt_amount as amount,transaction_heads_lang.head as category,receipt_details.name,receipt_details.phone,1 as from_section');
		$this->db->from('receipt');
		$this->db->join('receipt_details','receipt_details.receipt_id = receipt.id');
		$this->db->join('transaction_heads_lang','transaction_heads_lang.transactions_head_id = receipt_details.donation_category_id');
		$this->db->where('receipt.receipt_type','Mattu Varumanam');
	...
	function get_admin_income_mattuvarumanam_report($filterData){
		$this->db->select('daily_transactions.date,daily_transactions.amount,transaction_heads_lang.head as category,daily_transactions.name,0 as phone,2 as from_section');
	...
	function get_receipt_book_mattuvarumanam_report($filterData){
		$this->db->select('pos_receipt_book_used.actual_amount as amount,pos_receipt_book_used.date,book_type as category,0 as name,0 as phone,3 as from_section');
    ...
    function get_pooja_wise_report1($filterData){
        if($filterData['language']==1){
            $this->db->select('*,category_eng as category,pooja_name_eng as pooja_name_eng,sum(quantity) as count,sum(amount) as amount');
            }else{
                $this->db->select('*,category_alt as category,pooja_name_alt as pooja_name_eng,sum(quantity) as count,sum(amount) as amount');
            }
        //$this->db->select('*,sum(quantity) as count,sum(amount) as amount');
        $this->db->from('view_report_pooja_collection');
        $this->db->join('pooja_category','pooja_category.id=view_report_pooja_collection.pooja_category_id');
    ...
    function get_income_expense_report($filterData){
        ...
        sum(receipt_details.amount) as amount,1 as count,receipt_details.date as date,0 as type,receipt.receipt_type as receipt_type');
        $this->db->from('receipt_details');
        $this->db->join('receipt','receipt.id=receipt_details.receipt_id');
    ...
    function get_expensedetails_report($filterData){
        ...
        1 as count,daily_transactions.date as date,daily_transactions.transaction_heads_id,daily_transactions.temple_id,0 as receipt_type');
    ...
    function get_balitharaincome_report($filterData){
        ...
        1 as count,receipt_details.date as date,receipt_details.balithara_id,receipt.receipt_type as receipt_type');
        $this->db->from('receipt');
        $this->db->join('receipt_details','receipt_details.receipt_id = receipt.id');
    ...
    function get_hallincome_report($filterData){
        ...
        sum(receipt_details.amount) as amount,
        1 as count,receipt_details.date as date,3 as type,receipt_details.hall_master_id,receipt.receipt_type as receipt_type');
        $this->db->from('receipt');
        $this->db->join('receipt_details','receipt_details.receipt_id = receipt.id');
        $this->db->join('auditorium_master_lang','auditorium_master_lang.auditorium_master_id = receipt_details.hall_master_id');
        $this->db->where('auditorium_master_lang.lang_id=',$filterData['language']);
    ...
    function get_annadhanamincome_report($filterData){
        ...
        1 as count,receipt_details.date as date,4 as type,receipt.receipt_type as receipt_type');
        $this->db->from('receipt');
        $this->db->join('receipt_details','receipt_details.receipt_id = receipt.id');
        $this->db->where('receipt.receipt_type =','Annadhanam');
        $this->db->where('receipt.receipt_date >=',$filterData['from_date']);
    ...
    function get_praincome_report($filterData){
        ...
        1 as count,receipt_details.date as date,5 as type,receipt.receipt_type as receipt_type');
        $this->db->from('receipt_details');
        $this->db->join('receipt','receipt.id=receipt_details.receipt_id');
    ...
    function get_doantionincome_report($filterData){
        ...
        1 as count,receipt_details.date as date,receipt_details.donation_category_id,receipt.receipt_type as receipt_type');
        $this->db->from('receipt');
        $this->db->join('receipt_details','receipt_details.receipt_id = receipt.id');
        $this->db->join('donation_category_lang','donation_category_lang.donation_category_id = receipt_details.donation_category_id');
        $this->db->where('donation_category_lang.lang_id',$filterData['language']);
    ...
    function get_mattuvarumanamincome_report($filterData){
        ...
        sum(receipt_details.amount) as amount,
        1 as count,receipt_details.date as date,receipt_details.donation_category_id as mattuvarumanam_id,receipt.receipt_type as receipt_type');
        $this->db->from('receipt');
        $this->db->join('receipt_details','receipt_details.receipt_id = receipt.id');
        $this->db->join('transaction_heads_lang','transaction_heads_lang.transactions_head_id = receipt_details.donation_category_id');
        $this->db->where('transaction_heads_lang.lang_id =',$filterData['language']);
    ...
    function get_postalincome_report($filterData){
        ...
        1 as count,receipt.receipt_date as date,9 as type,receipt.receipt_type as receipt_type');
        $this->db->from('receipt');
        $this->db->where('receipt.receipt_status =','ACTIVE');
        $this->db->where('receipt.receipt_type =','Postal');
        $this->db->where('receipt.receipt_date >=',$filterData['from_date']);
    ...
    function get_assetincome_report($filterData){
        ...
        sum(receipt_details.amount) as amount,SUM(receipt_details.quantity) as total_quantity,
        1 as count,receipt_details.date as date,asset_master.asset_category_id,receipt.receipt_type as receipt_type');
        $this->db->from('receipt');
        $this->db->join('receipt_details','receipt_details.receipt_id=receipt.id');
    ...
    // function get_all_poojas($templeId,$languageId,$fromDate,$toDate){
    //     $this->db->select('pooja_master.id,pooja_master_lang.pooja_name,pooja_master.pooja_category_id,SUM(receipt_details.quantity) AS count,
    //     SUM(receipt_details.amount) AS amount');
    //     $this->db->from('pooja_master');
    //     $this->db->join('pooja_master_lang','pooja_master_lang.pooja_master_id=pooja_master.id');
    //     $this->db->join('receipt_details','receipt_details.pooja_master_id=pooja_master.id');
    //     $this->db->join('receipt','receipt.id=receipt_details.receipt_id');
    //     $this->db->join('pooja_category','pooja_category.id=pooja_master.pooja_category_id');
    ...
    //     }
    //     $this->db->group_by('pooja_master_lang.pooja_master_id');
	...
	function get_pooja_receipt_book_fixed_income_category($filterData){
        $this->db->select('pooja_master.id as pooja_master_id,sum(pos_receipt_book_used.actual_amount) as amount,0 as count,pos_receipt_book.rate,pooja_master_lang.pooja_name,pooja_category_lang.pooja_category_id as pooja_category_id,pooja_category_lang.category');
        $this->db->from('pos_receipt_book_used');
        $this->db->join('pos_receipt_book_items','pos_receipt_book_items.id = pos_receipt_book_used.enterd_book_id');
        $this->db->join('pos_receipt_book','pos_receipt_book.id = pos_receipt_book_items.book_id');
        $this->db->join('pooja_master','pooja_master.id = pos_receipt_book.item');
    ...
    function get_pooja_wise_fixed_receipt_book_report1($filterData){
        $this->db->select('pos_receipt_book_used.date,sum(pos_receipt_book_used.actual_amount) as amount,0 as count,pos_receipt_book.rate,pooja_master_lang.pooja_name,pooja_category.id as pooja_category_id,pooja_category_lang.category,pos_receipt_book.book_type as receipt_type');
        $this->db->from('pos_receipt_book_used');
        $this->db->join('pos_receipt_book_items','pos_receipt_book_items.id = pos_receipt_book_used.enterd_book_id');
        $this->db->join('pos_receipt_book','pos_receipt_book.id = pos_receipt_book_items.book_id');
        $this->db->join('pooja_master','pooja_master.id = pos_receipt_book.item');
    ...
    function get_pooja_wise_fixed_receipt_book_report2($filterData){
        $this->db->select('pos_receipt_book_used.date,
        sum(pos_receipt_book_used.actual_amount) as amount,
        0 as count,
        pos_receipt_book.rate,
        ,pos_receipt_book_lang.book as category,pos_receipt_book.book_type as receipt_type');
        $this->db->from('pos_receipt_book_used');
        $this->db->join('pos_receipt_book_items','pos_receipt_book_items.id = pos_receipt_book_used.enterd_book_id');
        $this->db->join('pos_receipt_book','pos_receipt_book.id = pos_receipt_book_items.book_id');
    ...
    function get_pooja_receipt_book_fixed_income($filterData){
        $this->db->select('sum(pos_receipt_book_used.actual_amount) as amount,pooja_category_lang.category,pooja_category_lang.pooja_category_id,pos_receipt_book.book_type as receipt_type');
        $this->db->from('pos_receipt_book_used');
        $this->db->join('pos_receipt_book_items','pos_receipt_book_items.id = pos_receipt_book_used.enterd_book_id');
        $this->db->join('pos_receipt_book','pos_receipt_book.id = pos_receipt_book_items.book_id');
        $this->db->join('pooja_master','pooja_master.id = pos_receipt_book.item');
    ...
    function get_prasadam_receipt_book_fixed_income($filterData){
        $this->db->select('sum(pos_receipt_book_used.actual_amount) as amount,item_category_lang.category,item_category_lang.item_category_id,pos_receipt_book.book_type as receipt_type');
        $this->db->from('pos_receipt_book_used');
        $this->db->join('pos_receipt_book_items','pos_receipt_book_items.id = pos_receipt_book_used.enterd_book_id');
        $this->db->join('pos_receipt_book','pos_receipt_book.id = pos_receipt_book_items.book_id');
    ...
    function get_other_receipt_book_income($filterData){
		$this->db->select('sum(pos_receipt_book_used.actual_amount) as amount,pos_receipt_book.book_type as category,pos_receipt_book.book_type as receipt_type');
        $this->db->from('pos_receipt_book_used');
        $this->db->join('pos_receipt_book_items','pos_receipt_book_items.id = pos_receipt_book_used.enterd_book_id');
        $this->db->join('pos_receipt_book','pos_receipt_book.id = pos_receipt_book_items.book_id');
        $this->db->where('pos_receipt_book_used.date >=',$filterData['from_date']);
    ...
    function get_prasadam_wise_report($filterData){
        if($filterData['language']==1){
        	$this->db->select('*,category_eng as category,name_alt as name_eng,sum(quantity) as count,sum(amount) as amount');
        }else{
            $this->db->select('*,category_alt as category,name_eng as name_eng,sum(quantity) as count,sum(amount) as amount');
        }
    ...
    function get_prasadam_wise_fixed_receipt_book_report($filterData){
        if($filterData['language']==1){
        $this->db->select('pos_receipt_book_used.date,sum(pos_receipt_book_used.actual_amount) as amount,0 as count,pos_receipt_book.rate,
        item_master_lang.name,item_category.id as item_category_id,item_category_lang.category');
        } else {
            $this->db->select('pos_receipt_book_used.date,sum(pos_receipt_book_used.actual_amount) as amount,0 as count,pos_receipt_book.rate,
            item_master_lang.name,item_category.id as item_category_id,item_category_lang.category');
        }
        ...
        if($filterData['temple_id'] != '1'){
            $this->db->where('pos_receipt_book_used.temple_id',$filterData['temple_id']);
           // $this->db->where('item_category.temple_id',$filterData['temple_id']);
        }
        $this->db->where('item_category_lang.lang_id',$filterData['language']);
        $this->db->where('item_master_lang.lang_id',$filterData['language']);
        $this->db->where('pos_receipt_book.book_type','Prasadam');
    ...
    function get_prasadam_wise_variable_receipt_book_report($filterData){
        if($filterData['language']==1){
        $this->db->select('pos_receipt_book_used.date,sum(pos_receipt_book_used.actual_amount) as amount,0 as count,
        pos_receipt_book.rate,item_master_lang.name,item_category.id as item_category_id,item_category_lang.category');
        }
        else{
        $this->db->select('pos_receipt_book_used.date,sum(pos_receipt_book_used.actual_amount) as amount,0 as count,
        pos_receipt_book.rate,item_master_lang.name,item_category.id as item_category_id,item_category_lang.category');  
    ...
    function get_asset_wise_report($filterData){
        if($filterData['language']==1){
            $this->db->select('*,asset_category_lang.category as category_eng,sum(quantity) as count,sum(receipt_details.amount) as amount');
            }else{
                $this->db->select('*,asset_category_lang.category as category_eng,sum(quantity) as count,sum(receipt_details.amount) as amount');
            }
            $this->db->from('receipt');
    ...
    function get_hall_wise_report($filterData){
        if($filterData['language']==1){
            $this->db->select('* ,hall_name_eng as name_eng,0 as count,sum(receipt_amount) as amount,receipt_type');
            }else{
                $this->db->select('*,hall_name_alt as name_eng,0 as count,sum(receipt_amount) as amount,receipt_type');
            }
        $this->db->from('view_report_hall');
    ...
    function get_annadanam_wise_report($filterData){
      ...
      //  $this->db->where('receipt.receipt_type',$filterData['temple_id']);
    ...
    function get_postal_wise_report($filterData){  		
		$this->db->select('receipt.receipt_type as category,sum(receipt_details.amount) as amount,receipt.receipt_date as date,
        sum(receipt_details.quantity) as count,9 as type,receipt.receipt_type as receipt_type,receipt_details.rate');
        $this->db->from('receipt');
        $this->db->join('receipt_details','receipt_details.receipt_id=receipt.id');
        $this->db->where('receipt.receipt_status =','ACTIVE');
        $this->db->where('receipt.receipt_type =','Postal');
    ...
    function get_mattu_wise_report($filterData){
        $this->db->select('transaction_heads_lang.head,sum(receipt_details.amount) as amount,transaction_heads_lang.transactions_head_id,receipt_details.donation_category_id as mattuvarumanam_id,receipt.receipt_type');
        $this->db->from('receipt');
        $this->db->join('receipt_details','receipt_details.receipt_id=receipt.id');
        $this->db->join('transaction_heads_lang','transaction_heads_lang.transactions_head_id=receipt_details.donation_category_id');
        $this->db->where('receipt_date >=',$filterData['from_date']);
    ...
    function get_expense_wise_report($filterData){
        $this->db->select('SUM(daily_transactions.amount) AS amount,transaction_heads_lang.head,transaction_heads_lang.transactions_head_id');
        $this->db->from('daily_transactions');
        $this->db->join('transaction_heads_lang','transaction_heads_lang.transactions_head_id=daily_transactions.transaction_heads_id');
        $this->db->where('daily_transactions.transaction_type','Income');
        $this->db->where('daily_transactions.date >=',$filterData['from_date']);
    ...
    function get_pooja1($templeId,$languageId,$fromDate,$toDate){
        $this->db->select('pooja_master_lang.pooja_master_id,pos_receipt_book_used.date,sum(pos_receipt_book_used.actual_amount) as total_amount,0 as count,pos_receipt_book.rate,pooja_master_lang.pooja_name,pooja_category.id as pooja_category_id,pooja_category_lang.category');
        $this->db->from('pos_receipt_book_used');
        $this->db->join('pos_receipt_book_items','pos_receipt_book_items.id = pos_receipt_book_used.enterd_book_id');
        $this->db->join('pos_receipt_book','pos_receipt_book.id = pos_receipt_book_items.book_id');
        $this->db->join('pooja_master','pooja_master.id = pos_receipt_book.item');
    ...
    function get_prasadam1($templeId,$languageId,$fromDate,$toDate){
        $this->db->select('item_master_lang.item_master_id,pos_receipt_book_used.date,sum(pos_receipt_book_used.actual_amount) as total_amount,0 as count,pos_receipt_book.rate,item_master_lang.name,item_category.id as item_category_id,item_category_lang.category');
        $this->db->from('pos_receipt_book_used');
        $this->db->join('pos_receipt_book_items','pos_receipt_book_items.id = pos_receipt_book_used.enterd_book_id');
        $this->db->join('pos_receipt_book','pos_receipt_book.id = pos_receipt_book_items.book_id');
        $this->db->join('item_master','item_master.id = pos_receipt_book.item');
	...
	function get_stock_out_quantity_report($filterData){
		$this->db->select('asset_master_lang.asset_name,asset_master_lang.asset_master_id,sum(stock_register_details.quantity) as total_quantity');
	...
	function get_scrap_asset_quantity($filterData){
		$this->db->select('asset_master_lang.asset_name,asset_master_lang.asset_master_id,sum(asset_rent_details.scrapped_quantity) as total_quantity');
