...
class Reports_model extends CI_Model {
   ...
   function get_pooja_wise_report($filterData){
        if($filterData['language']==1){
            $this->db->select('*,category_eng as category_eng,pooja_name_eng as pooja_name_eng,sum(quantity) as count,sum(amount) as amount,pooja_category_id');
            }else{
                $this->db->select('*,category_alt as category_eng,pooja_name_alt as pooja_name_eng,sum(quantity) as count,sum(amount) as amount,pooja_category_id');
            }
    ...
    function get_doantionincome_report($filterData){
        ...
        $this->db->where('donation_category_lang.lang_id = ',$filterData['language'] ,'AND receipt.receipt_type =','Donation');
        $this->db->where('receipt.receipt_date >=',$filterData['from_date']);
        $this->db->where('receipt.receipt_date <=',$filterData['to_date']);
        $this->db->where('receipt.temple_id',$filterData['temple_id']);
        $this->db->where('receipt.receipt_status =','ACTIVE');
