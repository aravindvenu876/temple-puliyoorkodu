...
class Daily_list_model extends CI_Model {
    ...
    function get_booked_pooja_list($date,$temple_id,$lang_id){
		...
		$this->db->select('
			opt_counter_receipt.receipt_date,
			opt_counter_receipt.receipt_time,
			opt_counter_receipt.id,
			opt_counter_receipt.receipt_no,
			pooja_master_lang.pooja_name,
			opt_counter_receipt_details.name,
			opt_counter_receipt_details.star,
			opt_counter_receipt_details.phone'
		);
        ...
        $this->db->join('pooja_master','pooja_master.id=opt_counter_receipt_details.pooja_master_id');
        $this->db->join('pooja_master_lang','pooja_master_lang.pooja_master_id = pooja_master.id');
        $this->db->join('pooja_category','pooja_category.id=pooja_master.pooja_category_id');
        ...
        $this->db->where('web_receipt_details.date',$date);
        $this->db->where('pooja_category.temple_id',$temple_id);
        $this->db->where('pooja_master_lang.lang_id',$lang_id);
        $this->db->where('pooja_master.status',1);
        $this->db->order_by('pooja_master_lang.pooja_name','asc');
    ...
    function get_booked_nivedya_list($date,$temple_id,$lang_id){
        $this->db->select('
            ...
            item_master.item_category_id,
            item_master.defined_quantity,
            unit.notation,
            pooja_master_lang.pooja_name,
            item_master_lang.name as item,
            receipt_details.name,
            receipt_details.star,
            pooja_master.id as poojaId,
            item_master.id as ItemId,
            receipt.id as receiptId,
            1 as counter_receipt
        ');
        $this->db->from('receipt');
        $this->db->join('receipt_details','receipt_details.receipt_id=receipt.id');
        $this->db->join('pooja_master','pooja_master.id=receipt_details.pooja_master_id');
		...
		$this->db->select('
			...
			pooja_master.id as poojaId,
			item_master.id as ItemId,
			opt_counter_receipt.id as receiptId,
            1 as counter_receipt
		');
        $this->db->from('opt_counter_receipt');
        $this->db->join('opt_counter_receipt_details','opt_counter_receipt_details.receipt_id=opt_counter_receipt.id');
        ...
        $this->db->where('opt_counter_receipt_details.date',$date);
        $this->db->where('opt_counter_receipt_details.prasadam_check',1);
        $this->db->where('pooja_category.temple_id',$temple_id);
        $this->db->where('pooja_master_lang.lang_id',$lang_id);
        $this->db->where('item_master_lang.lang_id',$lang_id);
        $this->db->where('item_category_lang.lang_id',$lang_id);
		...
		$this->db->select('
			...
			pooja_master.id as poojaId,
			item_master.id as ItemId,
			web_receipt_main.id as receiptId,
            0 as counter_receipt
		');
        $this->db->from('web_receipt_main');
        $this->db->join('web_receipt_details','web_receipt_details.receipt_id=web_receipt_main.id');
        $this->db->join('pooja_master','pooja_master.id=web_receipt_details.pooja_master_id');
        $this->db->join('pooja_master_lang','pooja_master_lang.pooja_master_id = pooja_master.id');
        $this->db->join('pooja_category','pooja_category.id=pooja_master.pooja_category_id');
        $this->db->join('web_pooja_prasadams','web_pooja_prasadams.pooja_id = web_receipt_details.pooja_master_id');
        $this->db->join('item_master','item_master.id = web_pooja_prasadams.prasadam_id');
        $this->db->join('item_master_lang','item_master_lang.item_master_id = item_master.id');
        $this->db->join('item_category','item_category.id = item_master.item_category_id');
        $this->db->join('item_category_lang','item_category_lang.item_category_id = item_category.id');
        $this->db->join('unit','unit.id = item_category.unit');
        ...
        $this->db->where('item_category_lang.lang_id',$lang_id);
        $this->db->where('pooja_master.status',1);
        $this->db->where('web_pooja_prasadams.status',1);
        $this->db->order_by('item_master_lang.name','asc');
        $data3 = $this->db->get()->result();
		return array_merge($data1, $data2, $data3);
    }

    function get_additional_booked_prasadam_list($date,$temple_id,$lang_id,$poojaId,$ItemId,$name,$star){
        ...
        $this->db->where('receipt_details.name',$name);
        $this->db->where('receipt_details.star',$star);
        $this->db->where('receipt_details.date',$date);
        $this->db->where('item_master.id',$ItemId);
        $this->db->where('pooja_master.id',$poojaId);
        $this->db->where('receipt_details.prasadam_check',1);
        $this->db->where('pooja_category.temple_id',$temple_id);
        $this->db->where('pooja_master_lang.lang_id',$lang_id);
        $this->db->where('item_master_lang.lang_id',$lang_id);
    ...
    function get_additional_booked_prasadam_list1($date,$temple_id,$lang_id,$poojaId,$ItemId,$name,$star){
		$this->db->select('
			opt_counter_receipt.receipt_date,
			opt_counter_receipt.receipt_time,
			item_category_lang.category,
			...
			opt_counter_receipt_details.name,
			opt_counter_receipt_details.star,
			opt_counter_receipt_details.quantity
		');
        $this->db->from('opt_counter_receipt');
        $this->db->join('opt_counter_receipt_details','opt_counter_receipt_details.receipt_id=opt_counter_receipt.id');
        $this->db->join('item_master','item_master.id = opt_counter_receipt_details.item_master_id');
        $this->db->join('item_master_lang','item_master_lang.item_master_id = item_master.id');
        $this->db->join('item_category','item_category.id = item_master.item_category_id');
        $this->db->join('item_category_lang','item_category_lang.item_category_id = item_category.id');
        $this->db->join('pooja_prasadam_mapping','pooja_prasadam_mapping.item_id = item_master.id');
        ...
        $this->db->join('pooja_category','pooja_category.id=pooja_master.pooja_category_id');
        $this->db->join('unit','unit.id = item_category.unit');
        $this->db->where('opt_counter_receipt.receipt_type','Prasadam');
        $this->db->where('opt_counter_receipt.api_type',"Pooja");
        $this->db->where('opt_counter_receipt.receipt_status','ACTIVE');
        $this->db->where('opt_counter_receipt_details.name',$name);
        $this->db->where('opt_counter_receipt_details.star',$star);
        $this->db->where('opt_counter_receipt_details.date',$date);
        $this->db->where('item_master.id',$ItemId);
        $this->db->where('pooja_master.id',$poojaId);
        $this->db->where('opt_counter_receipt_details.prasadam_check',1);
    ...
    function get_additional_booked_prasadam_list1_new($date,$temple_id,$lang_id){
		$this->db->select('
			opt_counter_receipt.receipt_date,
			opt_counter_receipt.receipt_time,
			item_category_lang.category,
			...
			opt_counter_receipt_details.star,
			opt_counter_receipt_details.quantity,
            item_master.id as prasadam_id,
            pooja_master.id as pooja_id
		');
        $this->db->from('opt_counter_receipt');
        $this->db->join('opt_counter_receipt_details','opt_counter_receipt_details.receipt_id=opt_counter_receipt.id');
        $this->db->join('item_master','item_master.id = opt_counter_receipt_details.item_master_id');
