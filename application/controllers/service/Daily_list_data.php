...
class Daily_list_data extends REST_Controller {
    ...
    function get_nivedya_list_post_old(){
        $date = date('Y-m-d',strtotime($this->post('date')));
        if($date > date('Y-m-d')){
            $data['date_check'] = 1;
        }else{
    ...
    function get_nivedya_list_post(){
        ...
        $listData['daily_nivedya_list'] = $this->Daily_list_model->get_daily_mandatory_nivedyas($this->templeId,$this->languageId);
        $booked_nivedya_list            = $this->Daily_list_model->get_booked_nivedya_list($date,$this->templeId,$this->languageId);
        $dditional_booked_list          = $this->Daily_list_model->get_additional_booked_prasadam_list1_new($date,$this->templeId,$this->languageId);
        foreach($booked_nivedya_list as $key => $row){
            $total_quantity = $row->defined_quantity;
            if($row->counter_receipt == 1){
                foreach($dditional_booked_list as $val){
                    if($row->poojaId == $val->pooja_id && $row->ItemId == $val->prasadam_id && $row->name == $val->name && $row->star == $val->star){
                        $total_quantity = $row->defined_quantity + ($data2['defined_quantity']*$data2['quantity']);
                    }
                }
            }
            $booked_nivedya_list[$key]->total_quantity = $total_quantity;
        }
        $listData['booked_nivedya_list'] = $booked_nivedya_list;
        usort($listData['booked_nivedya_list'], function($obj1, $obj2) {
