...
class Account_sync_data extends REST_Controller {
                ...
                if($row->pooja_type == "Normal"){
                    ...
                    if($row->payment_type == "ADVANCE"){                             
                        ...
                        if($row->description == "Aavahanam Pooja"){                            
                            ...
                            $getAvahanamAdvance = $this->db->select('*')->where('receipt_identifier',$row->receipt_identifier)->where('payment_type','ADVANCE')->get('receipt')->row_array();
                            $accountEntryMain = array();
                            if($row->receipt_status == 'DRAFT'){
                                $accountEntryMain['status'] = "TEMP";
                            }
                            ...
                            if($row->receipt_status == 'CANCELLED'){
                                ...
                                $accountEntryMain['head'] = "";
								$accountEntryMain['table'] = "pooja_master";							
								$accountEntryMain['amount'] = $row->receipt_amount + $getAvahanamAdvance['receipt_amount'];
								$accountEntryMain['amount4'] = $row->receipt_amount;
								$accountEntryMain['amount5'] = $getAvahanamAdvance['receipt_amount'];
								$accountEntryMain['description'] = "";
								$accountEntryMain['accountType'] = "Prathima Aavahanam Final";
