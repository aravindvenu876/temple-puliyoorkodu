...
class Reports_data extends REST_Controller {
	...
	function get_allincome_report_post(){
        ...
        $data['balanceToDeposit'] = $this->Reports_model->get_balance_to_be_deposited($dataFilter);
	...
	function get_income_expense_excel_get(){
		...
		// $mpdf->Output();
		//Excel Code
		$balanceToDeposit 		= $data['balanceToDeposit'];
		...
		$totalBankDeposit 		= $data['totalBankDeposit'];
		$balanceToDeposit 		= $data['balanceToDeposit'];
        ...
        $objPHPExcel->getActiveSheet()->SetCellValue('C8', $this->lang->line('cash'));
        $objPHPExcel->getActiveSheet()->SetCellValue('D8', $this->lang->line('card'));
        $objPHPExcel->getActiveSheet()->SetCellValue('E8', $this->lang->line('mo'));
        $objPHPExcel->getActiveSheet()->SetCellValue('F8', $this->lang->line('cheque'));
        $objPHPExcel->getActiveSheet()->SetCellValue('G8', $this->lang->line('dd'));
        $objPHPExcel->getActiveSheet()->SetCellValue('H8', $this->lang->line('online'));
		$objPHPExcel->getActiveSheet()->SetCellValue('I8', $this->lang->line('total'));
		$rowCount = 9;
		$i=0;
		...
		$ulsavamCheque = 0;
		$ulsavamOnline = 0;
		// echo "<pre>";print_r($incomeReport);die();
			...
			if($row->item_section_id =="report167"){
				...
				if($amount1 != 0){
                    if($row->receipt_type == "Pooja" && $row->pooja_category_id ==$pooja_category_id){
						...
						$objPHPExcel->getActiveSheet()->SetCellValue('C'.$rowCount, number_format($cash1, 2, '.', ''));
						$objPHPExcel->getActiveSheet()->SetCellValue('D'.$rowCount, number_format($card, 2, '.', ''));
						$objPHPExcel->getActiveSheet()->SetCellValue('E'.$rowCount, number_format($mo, 2, '.', ''));
						$objPHPExcel->getActiveSheet()->SetCellValue('F'.$rowCount, number_format($cheque, 2, '.', ''));
						$objPHPExcel->getActiveSheet()->SetCellValue('G'.$rowCount, number_format($dd, 2, '.', ''));
						$objPHPExcel->getActiveSheet()->SetCellValue('H'.$rowCount, number_format($online, 2, '.', ''));
						$objPHPExcel->getActiveSheet()->SetCellValue('I'.$rowCount, number_format($amount1, 2, '.', ''));
						$rowCount++;
					} 
				} 
			} 
		...
		if($ulsavamAmount >0){ 
			...
			$objPHPExcel->getActiveSheet()->SetCellValue('C'.$rowCount, number_format($ulsavamCash, 2, '.', ''));
			$objPHPExcel->getActiveSheet()->SetCellValue('D'.$rowCount, number_format($ulsavamCard, 2, '.', ''));
			$objPHPExcel->getActiveSheet()->SetCellValue('E'.$rowCount, number_format($ulsavamMo, 2, '.', ''));
			$objPHPExcel->getActiveSheet()->SetCellValue('F'.$rowCount, number_format($ulsavamCheque, 2, '.', ''));
			$objPHPExcel->getActiveSheet()->SetCellValue('G'.$rowCount, number_format($ulsavamDd, 2, '.', ''));
			$objPHPExcel->getActiveSheet()->SetCellValue('H'.$rowCount, number_format($ulsavamOnline, 2, '.', ''));
			$objPHPExcel->getActiveSheet()->SetCellValue('I'.$rowCount, number_format($ulsavamAmount, 2, '.', ''));
			$rowCount++;
		}
		foreach($receiptBookIncome as $key => $row1){
			if(!in_array($key,$indexStoreArray)){
				...
				$objPHPExcel->getActiveSheet()->SetCellValue('C'.$rowCount, number_format($row1->amount, 2, '.', ''));
				...
				$objPHPExcel->getActiveSheet()->SetCellValue('I'.$rowCount, number_format($row1->amount, 2, '.', ''));
				$rowCount++;		
			}
		}
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('C'.$rowCount, number_format($cashIncome, 2, '.', ''));
		$objPHPExcel->getActiveSheet()->SetCellValue('D'.$rowCount, number_format($cardIncome, 2, '.', ''));
		$objPHPExcel->getActiveSheet()->SetCellValue('E'.$rowCount, number_format($moIncome, 2, '.', ''));
		$objPHPExcel->getActiveSheet()->SetCellValue('F'.$rowCount, number_format($chequeIncome, 2, '.', ''));
		$objPHPExcel->getActiveSheet()->SetCellValue('G'.$rowCount, number_format($ddIncome, 2, '.', ''));
		$objPHPExcel->getActiveSheet()->SetCellValue('H'.$rowCount, number_format($onlineIncome, 2, '.', ''));
		$objPHPExcel->getActiveSheet()->SetCellValue('I'.$rowCount, number_format($ttIncome, 2, '.', ''));
		$rowCount++;	
			...
			if($row->type == "To"){ 
				...
				if($headId == 0){
					...
					if($headId != $row->sub_head_id){
						...
						$objPHPExcel->getActiveSheet()->SetCellValue('B'.$rowCount, number_format($headAmount, 2, '.', ''));
						$rowCount++;
						$headId 	= $row->sub_head_id;
						$headName 	= $row->head;
						$headAmount = $row->credit;
		...
		$rowCount++;
		$objPHPExcel->getActiveSheet()->SetCellValue('A'.$rowCount, $this->lang->line('total_amount'));
		$objPHPExcel->getActiveSheet()->SetCellValue('B'.$rowCount, number_format($totalJournalAmount, 2, '.', ''));
		$rowCount++;
		foreach($accountReport as $row){ 
			...
			$objPHPExcel->getActiveSheet()->SetCellValue('B'.$rowCount, number_format($row->totalWithdrawal, 2, '.', ''));
			$rowCount++;
			$objPHPExcel->getActiveSheet()->SetCellValue('A'.$rowCount, $this->lang->line('petty_cash_withdrawal').'('.$row->bank_eng.')');
			$objPHPExcel->getActiveSheet()->SetCellValue('B'.$rowCount, number_format($row->pettyCashWithdrawal, 2, '.', ''));
			$rowCount++;
		}
		foreach ($bankWithdrawalSplit as $row){
			if($row->type == "PETTY CASH WITHDRAWAL"){
				...
				$objPHPExcel->getActiveSheet()->SetCellValue('B'.$rowCount, number_format($row->amount, 2, '.', ''));
				$rowCount++;
			}
		}
		$objPHPExcel->getActiveSheet()->SetCellValue('A'.$rowCount, $this->lang->line('total')." ".$this->lang->line('Withdrawal'));
		$objPHPExcel->getActiveSheet()->SetCellValue('B'.$rowCount, number_format($bankWithdrawal, 2, '.', ''));
		$rowCount++;
		$objPHPExcel->getActiveSheet()->SetCellValue('A'.$rowCount, $this->lang->line('total')." FD ".$this->lang->line('varavu'));
		$objPHPExcel->getActiveSheet()->SetCellValue('B'.$rowCount, number_format($total_fd_to_sb['amount'], 2, '.', ''));
		$rowCount++;
		$objPHPExcel->getActiveSheet()->SetCellValue('A'.$rowCount, $this->lang->line('Income_By_Receipts'));
		$objPHPExcel->getActiveSheet()->SetCellValue('B'.$rowCount, number_format($totalIncome, 2, '.', ''));
		$rowCount++;
		$totalIncomeAmount = $bankWithdrawal + $totalIncome + $total_fd_to_sb['amount'] + $totalJournalAmount;
		$objPHPExcel->getActiveSheet()->SetCellValue('A'.$rowCount, $this->lang->line('total_amount'));
		$objPHPExcel->getActiveSheet()->SetCellValue('B'.$rowCount, number_format($totalIncomeAmount, 2, '.', ''));
		$rowCount++;
		$objPHPExcel->getActiveSheet()->mergeCells('A'.$rowCount.':I'.$rowCount);
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('C'.$rowCount, $this->lang->line('account'));
		$objPHPExcel->getActiveSheet()->SetCellValue('D'.$rowCount, $this->lang->line('amount'));
		$rowCount++;	
		$i=1;
		$sum=0;
		$total = $pettyCashOpen; 
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('D'.$rowCount, number_format($pettyCashOpen, 2, '.', ''));
		$rowCount++;	
		foreach($accountReport as $row){ 
			$i++;
			$sum=$row->opening;
			...
			$objPHPExcel->getActiveSheet()->SetCellValue('C'.$rowCount, $row->account_no);
			$objPHPExcel->getActiveSheet()->SetCellValue('D'.$rowCount, $row->opening);
			$rowCount++;	
		}
		$objPHPExcel->getActiveSheet()->SetCellValue('A'.$rowCount, '');
		$objPHPExcel->getActiveSheet()->SetCellValue('B'.$rowCount, '');
		$objPHPExcel->getActiveSheet()->SetCellValue('C'.$rowCount, $this->lang->line('total').' SB');
		$objPHPExcel->getActiveSheet()->SetCellValue('D'.$rowCount, number_format(($total - $pettyCashOpen), 2, '.', ''));
		$rowCount++;
		$objPHPExcel->getActiveSheet()->mergeCells('A'.$rowCount.':I'.$rowCount);
			...
			if($row->st == 1){
				if($i != 0){
					if($defaultBankId != $row->bank_id){ 
						$objPHPExcel->getActiveSheet()->SetCellValue('A'.$rowCount, '');
						$objPHPExcel->getActiveSheet()->SetCellValue('B'.$rowCount, '');
						$objPHPExcel->getActiveSheet()->SetCellValue('C'.$rowCount, $this->lang->line('total')." ".$defaultBank .'FD');
						$objPHPExcel->getActiveSheet()->SetCellValue('D'.$rowCount, number_format($bankSum, 2, '.', ''));
						$rowCount++;
						$bankSum = 0;
					}
				}
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('A'.$rowCount, '');
		$objPHPExcel->getActiveSheet()->SetCellValue('B'.$rowCount, '');
		$objPHPExcel->getActiveSheet()->SetCellValue('C'.$rowCount, $this->lang->line('total').' FD');
		$objPHPExcel->getActiveSheet()->SetCellValue('D'.$rowCount, number_format($totalSum, 2, '.', ''));
		$rowCount++;
		$objPHPExcel->getActiveSheet()->mergeCells('A'.$rowCount.':I'.$rowCount);
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('A'.$rowCount, $this->lang->line('sl'));
		$objPHPExcel->getActiveSheet()->SetCellValue('B'.$rowCount, $this->lang->line('item'));
		$objPHPExcel->getActiveSheet()->SetCellValue('C'.$rowCount, $this->lang->line('cash'));
		$objPHPExcel->getActiveSheet()->SetCellValue('D'.$rowCount, $this->lang->line('card'));
		$objPHPExcel->getActiveSheet()->SetCellValue('E'.$rowCount, $this->lang->line('mo'));
		$objPHPExcel->getActiveSheet()->SetCellValue('F'.$rowCount, $this->lang->line('cheque'));
		$objPHPExcel->getActiveSheet()->SetCellValue('G'.$rowCount, $this->lang->line('dd'));
		$objPHPExcel->getActiveSheet()->SetCellValue('H'.$rowCount, $this->lang->line('online'));
		$objPHPExcel->getActiveSheet()->SetCellValue('I'.$rowCount, $this->lang->line('total'));
		$rowCount++;
		$i=0;
		$cashExpense = 0;
		$cardExpense = 0;
		...
		foreach($expenseReport as $row){
			...
			$objPHPExcel->getActiveSheet()->SetCellValue('A'.$rowCount, $i);
			$objPHPExcel->getActiveSheet()->SetCellValue('B'.$rowCount, $row->category);
			$objPHPExcel->getActiveSheet()->SetCellValue('C'.$rowCount, number_format($cash, 2, '.', ''));
			$objPHPExcel->getActiveSheet()->SetCellValue('D'.$rowCount, number_format($card, 2, '.', ''));
			$objPHPExcel->getActiveSheet()->SetCellValue('E'.$rowCount, number_format($mo, 2, '.', ''));
			$objPHPExcel->getActiveSheet()->SetCellValue('F'.$rowCount, number_format($cheque, 2, '.', ''));
			$objPHPExcel->getActiveSheet()->SetCellValue('G'.$rowCount, number_format($dd, 2, '.', ''));
			$objPHPExcel->getActiveSheet()->SetCellValue('H'.$rowCount, number_format($online, 2, '.', ''));
			$objPHPExcel->getActiveSheet()->SetCellValue('I'.$rowCount, number_format($amount, 2, '.', ''));
			$rowCount++;
		}
		$objPHPExcel->getActiveSheet()->mergeCells('A'.$rowCount.':B'.$rowCount);
		$objPHPExcel->getActiveSheet()->SetCellValue('A'.$rowCount, $this->lang->line('total_amount'));
		$objPHPExcel->getActiveSheet()->SetCellValue('C'.$rowCount, number_format($cashExpense, 2, '.', ''));
		$objPHPExcel->getActiveSheet()->SetCellValue('D'.$rowCount, number_format($cardExpense, 2, '.', ''));
		$objPHPExcel->getActiveSheet()->SetCellValue('E'.$rowCount, number_format($moExpense, 2, '.', ''));
		$objPHPExcel->getActiveSheet()->SetCellValue('F'.$rowCount, number_format($chequeExpense, 2, '.', ''));
		$objPHPExcel->getActiveSheet()->SetCellValue('G'.$rowCount, number_format($ddExpense, 2, '.', ''));
		$objPHPExcel->getActiveSheet()->SetCellValue('H'.$rowCount, number_format($onlineExpense, 2, '.', ''));
		$objPHPExcel->getActiveSheet()->SetCellValue('I'.$rowCount, number_format($ttExpense, 2, '.', ''));
		$rowCount++;
		$objPHPExcel->getActiveSheet()->mergeCells('A'.$rowCount.':I'.$rowCount);
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('A'.$rowCount, $this->lang->line('sl'));
		$objPHPExcel->getActiveSheet()->SetCellValue('B'.$rowCount, $this->lang->line('bank'));
		$objPHPExcel->getActiveSheet()->SetCellValue('C'.$rowCount, 'SB '.$this->lang->line('Deposit'));
		$objPHPExcel->getActiveSheet()->SetCellValue('D'.$rowCount, 'FD '.$this->lang->line('Deposit'));
		$rowCount++;
		$ikl = 0;
		foreach($accountReport as $row){ 
			$ikl++;
			...
			$objPHPExcel->getActiveSheet()->SetCellValue('B'.$rowCount, $row->bank_eng);
			$objPHPExcel->getActiveSheet()->SetCellValue('C'.$rowCount, number_format($row->totalDeposit, 2, '.', ''));
			$objPHPExcel->getActiveSheet()->SetCellValue('D'.$rowCount, number_format($row->totalFDDeposit, 2, '.', ''));
			$rowCount++;
		}
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('D'.$rowCount, number_format($bankDeposit, 2, '.', ''));
		$rowCount++;
		$objPHPExcel->getActiveSheet()->mergeCells('A'.$rowCount.':C'.$rowCount);
		$objPHPExcel->getActiveSheet()->SetCellValue('A'.$rowCount, $this->lang->line('total')." FD ".$this->lang->line('Deposit'));
		$objPHPExcel->getActiveSheet()->SetCellValue('D'.$rowCount, number_format($total_sb_to_fd['amount'], 2, '.', ''));
		$rowCount++;
		$objPHPExcel->getActiveSheet()->SetCellValue('A'.$rowCount, $this->lang->line('total')." ".$this->lang->line('Deposit'));
		$objPHPExcel->getActiveSheet()->SetCellValue('B'.$rowCount, number_format($totalBankDeposit, 2, '.', ''));
		$rowCount++;
		$objPHPExcel->getActiveSheet()->SetCellValue('A'.$rowCount, $this->lang->line('Expense_Vouchers'));
		$objPHPExcel->getActiveSheet()->SetCellValue('B'.$rowCount, number_format($totalVoucherExpense, 2, '.', ''));
		$rowCount++;
		$pettyCashSpent = $cashExpense + $moExpense;
		$objPHPExcel->getActiveSheet()->SetCellValue('A'.$rowCount, $this->lang->line('petty_cash_spent'));
		$objPHPExcel->getActiveSheet()->SetCellValue('B'.$rowCount, number_format($pettyCashSpent, 2, '.', ''));
		$rowCount++;
		$objPHPExcel->getActiveSheet()->SetCellValue('A'.$rowCount, $this->lang->line('Deposit_Balance').' - '.$to_date);
		$objPHPExcel->getActiveSheet()->SetCellValue('B'.$rowCount, number_format($balanceToDeposit, 2, '.', ''));
		$rowCount++;
		$totalExpenseAmount = $totalBankDeposit + $totalVoucherExpense + $totalJournalAmount;
		$objPHPExcel->getActiveSheet()->SetCellValue('A'.$rowCount, $this->lang->line('total_amount'));
		$objPHPExcel->getActiveSheet()->SetCellValue('B'.$rowCount, number_format($totalExpenseAmount, 2, '.', ''));
		$rowCount++;
		$objPHPExcel->getActiveSheet()->mergeCells('A'.$rowCount.':I'.$rowCount);
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('B'.$rowCount, $this->lang->line('petty_cash'));
		$objPHPExcel->getActiveSheet()->SetCellValue('C'.$rowCount, '');
		$objPHPExcel->getActiveSheet()->SetCellValue('D'.$rowCount, number_format($pettyCashClose, 2, '.', ''));
		$rowCount++;
		foreach($accountReport as $row){ 
			$i++;
			$sum=$row->closing;
			...
			$objPHPExcel->getActiveSheet()->SetCellValue('B'.$rowCount, $row->bank_eng);
			$objPHPExcel->getActiveSheet()->SetCellValue('C'.$rowCount, $row->account_no);
			$objPHPExcel->getActiveSheet()->SetCellValue('D'.$rowCount, $row->closing);
			$rowCount++;
		}	
		$objPHPExcel->getActiveSheet()->SetCellValue('A'.$rowCount, '');
		$objPHPExcel->getActiveSheet()->SetCellValue('B'.$rowCount, '');
		...
		// $reportTitle = $temple." ".$from_date." to ".$to_date." taken on ".date('Ymd');
