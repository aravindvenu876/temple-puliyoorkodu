...
class Reports_data extends REST_Controller {
    ...
    function get_poojawise_report_post(){
        ...
        // $reportFixedReceiptBook_1 = $this->Reports_model->get_pooja_wise_fixed_receipt_book_report_1($dataFilter);
        // $reportVariableReceiptBook_1 = $this->Reports_model->get_pooja_wise_variable_receipt_book_report_1($dataFilter);
        // $reportData_1 = array_merge($report,$reportFixedReceiptBook,$reportVariableReceiptBook);
   ...
   // echo $this->db->last_query();
    ...
    // echo $this->db->last_query();
		...
		// $oldReceiptId = 0;
		// $totalDiscount = 0;
		// foreach($report8 as $key => $row){
		// 	if($oldReceiptId == $row->id){
		// 		if($totalDiscount != 0){
		// 			if($row->amount < $totalDiscount){
		// 				$totalDiscount = $totalDiscount - $row->amount;
		// 				$report8[$key]->amount = 0;
		// 			}else{
		// 				$report8[$key]->amount = $row->amount - $totalDiscount;
		// 				$totalDiscount = 0;
		// 			}
		// 		}
		// 	}else{
		// 		$totalDiscount = $this->Reports_model->get_totalassetdiscount($row->id)['actual_discount'];
		// 		$oldReceiptId = $row->id;
		// 		if($row->amount < $totalDiscount){
		// 			$totalDiscount = $totalDiscount - $row->amount;
		// 			$report8[$key]->amount = 0;
		...
		// $data['assetDiscount'] = $this->Reports_model->get_totalassetdiscount($dataFilter);
   ...
   function get_bank_balance_report_post(){
    ...
    function get_prasadamwise_report_post(){
        ...
        $dataFilter['language'] = $this->languageId;
        $dataFilter['temple_id']=$this->templeId;
        $report1 = $this->Reports_model->get_asset_wise_report($dataFilter);
        ...
        // usort($report1, function($obj1, $obj2) {
        ...
        $data['report_hall']= $this->Reports_model->get_hall_wise_report($dataFilter);
		...
		foreach($datamattureceipt as $key => $row){
			$datamattureceipt[$key]->category = "Mattu Varumanam (Receipt Book)";
		}
        ...
        usort($report_merge, function($obj1, $obj2) {
            return $obj1->transactions_head_id - $obj2->transactions_head_id;
        });
    ...
    function get_all_pdf_get(){
        ...
        $dataFilter['templesub_id']=3; 
        $report_2 = $this->Reports_model->get_pooja_wise_report_1($dataFilter);
      ...
      // echo '<pre>'; print_r($data['report']);die();
        ...
        usort($report1, function($obj1, $obj2) {
            return $obj1->asset_category_id - $obj2->asset_category_id;
        });
        $data['report1'] = $report1;
        // postal
        $data['report_bali']= $this->Reports_model->get_balithara_wise_report($dataFilter);
