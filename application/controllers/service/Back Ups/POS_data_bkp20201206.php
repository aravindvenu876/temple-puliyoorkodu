...
class POS_data extends REST_Controller {
    ...
    function print_session_receipts_get(){
        ...
        $listData['session'] = $this->Pos_model->get_counter_detail($sessionId);
        $listData['receipts'] = $this->Pos_model->get_receipt_details($sessionId);
        $this->languageId=1;
        $listData['temple'] = $this->General_Model->get_temple_information($this->templeId,$this->languageId)['temple'];
        $this->load->library('Pdf');
        $this->load->view("pos/session_receipt_pdf", $listData);
    }
