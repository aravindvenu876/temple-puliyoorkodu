...
class Account_basic_data extends REST_Controller {
    ...
    function edit_accounting_head_get(){
		...
		foreach($mappedItems as $key => $row){
			$tempData = $this->Account_model->account_table_name($row->table_id);
			if(empty($tempData)){
    ...
    function get_accounting_groups_get(){
        $filterList = array();
		$filterList['head'] = $this->input->get_post('head', TRUE);
		$filterList['parent_head'] = $this->input->get_post('parent_head', TRUE);
        $iDisplayStart = $this->input->get_post('iDisplayStart', TRUE);
        $iDisplayLength = $this->input->get_post('iDisplayLength', TRUE);
        $iSortCol_0 = $this->input->get_post('iSortCol_0', TRUE);
        $iSortingCols = $this->input->get_post('iSortingCols', TRUE);
    ...
    function sync_receipt_with_accounting_entries_get(){
		...
		foreach($annadhanamReceipts as $row){
			if($row->pooja_type == "Advance"){  
				...
				// 	$accountEntryMain['amount2'] = $row->amount;
		...
		$this->db->where('receipt.receipt_date >=',$fromdate);
		$this->db->where('receipt.receipt_date <=',$todate);
		$this->db->update('receipt',$updateArray);
	}

	function get_normal_annadhanam_get(){
		$fromdate = date('Y-m-d',strtotime($this->get('fromdate')));
		$todate = date('Y-m-d',strtotime($this->get('todate')));
		$card 	= $this->Account_model->get_bank_account_ledger('card',$this->templeId);
		$cheque = $this->Account_model->get_bank_account_ledger('cheque',$this->templeId);
	...
	function update_bank_tally_get(){
		$data = $this->db->select('*')->where_in('type',array('WITHDRAWAL','PETTY CASH WITHDRAWAL'))->where('temple_id',$this->templeId)->where('date <=','2019-09-30')->get('bank_transaction')->result();
		echo "<table>";
    ...
    function generate_tally_xml_get(){
        ...
        $TallyData = $this->db->limit(1000)->get()->result();
        $requestXML = "";
        $requestXML .= "<ENVELOPE>\n";
        $requestXML .= "<HEADER>\n";
		...
		$xmlFileName = "VoucherFile".date('his');
        $fp = fopen($_SERVER['DOCUMENT_ROOT'] . "/temple/tally_files/".$directory."/".$xmlFileName.".xml","wb");
	...
	function add_joureal_entry_post(){
			...
			for($i=1;$i<=$count;$i++){
				$total_amount=$total_amount+$this->input->post('amount_'.$i);
			}
			if($amount!=$total_amount){
				echo json_encode(['message' => 'error','viewMessage' => 'Ledger amount and sum of the sub entry amount is not same']);
				return;
			}
			if($type=="credit"){
