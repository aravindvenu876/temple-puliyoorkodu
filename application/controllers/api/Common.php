...
class Common extends REST_Controller {
    ...
    function malayalam_calendar_list_post(){
        $from_date = date('Y-m-d',strtotime($this->requestData->from_date));
        $to_date = date('Y-m-d',strtotime($this->requestData->to_date));
        $mal_dates = $this->common_model->malayalam_calendar_list($from_date, $to_date);
        $this->responseData['data']['malayalam'] = $mal_dates;
        $this->responseData['message'] = "Malayalam Dates";
        $this->response($this->responseData);
    }
