...
class Receipt extends REST_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('api/api_new_model');
    ...
    function current_session_receipt_post(){
        ...
        $receipt_no = $this->requestData->receipt_no;
        $value_count = $this->requestData->value_count;
        $page_no = $this->requestData->page_no;
        $receipts = $this->api_new_model->current_session_receipts($session_id, $receipt_no);
        $total_count = count($receipts);
        $page_count = floor($total_count/$value_count);
        $reminder_count = $total_count%$value_count;
        if($reminder_count != 0)
            $page_count = $page_count + 1;
        $result_data = [];
        $i = 0;
        $page_end = $page_no * $value_count;
        $page_start = $page_end - $value_count;
        ...
        $resp_data = array(
            ...
            'receipts'      => $result_data
        );
        $this->responseData['status'] = TRUE;
        $this->responseData['message'] = "Current Session Receipt List";
        $this->responseData['data'] = $resp_data;
        $this->response($this->responseData);
    }

    function counter_receipts_post(){
        $receipt_date   = date('Y-m-d',strtotime($this->requestData->receipt_date));
        $receipt_no     = $this->requestData->receipt_no;
        $temple_id      = $this->requestData->temple_id;
        $page_no        = $this->requestData->page_no;
        $page_item_count= $this->requestData->value_count;
        $total_receipts = $this->api_model->get_counter_receipts($temple_id, $receipt_date, $receipt_no);
        $starting_index = (($page_no - 1) * $page_item_count) + 1;
        $ending_index   = $page_no * $page_item_count;
        $total_items    = count($total_receipts);
        $total_pages    = floor($total_items / $page_item_count);
        $reminder_count = $total_items % $page_item_count;
        if($reminder_count > 0){
        ...
        $this->responseData['message']              = "Current Session Receipt List";
        $this->responseData['data']['total_count']  = $total_items;
        $this->responseData['data']['page_count']   = $total_pages;
        $this->responseData['data']['receipts']     = $receiptData;
        $this->response($this->responseData);
    }

    function counter_receipts_old_post(){
                    ...
                    if($this->api_model->cancel_individual_receipt($actualReceiptid,$this->requestData)){
                        if($receiptDetail['accounting_status'] == 1){
							...
							foreach($receiptPoojaDetails as $row){
								$this->db->select('receipt.receipt_no,receipt_details.pooja_master_id,receipt_details.amount,pooja_category.temple_id');
								...
								$this->db->where('receipt.id',$row->receipt_id);
								$receiptDetails = $this->db->get()->row_array();
								if($this->requestData->temple_id == '1'){
									if($receiptDetails['temple_id'] != '1'){
										...
										$accountEntryMain['voucher_no'] = $receiptDetails['receipt_no'];
										$accountEntryMain['date'] = $row->receipt_date;
										$accountEntryMain['description'] = "";
										$this->accounting_entries->accountingEntry($accountEntryMain);
									}else{
                        ...
                        foreach($receipts as $row){
                            ...
                            if($row->receipt_type == "Hall"){
                                ...
                                foreach($prasadamDetails as $val){
                                    ...
                                    $this->Item_model->update_item($val->item_master_id,$updatePrasadmData, $prasadamData['ledger_id']);
                                    /**Accounting Entry Start*/
                                    if($row->accounting_status == 1){
                                        $accountEntryMain = array();
                                        $accountEntryMain['temple_id'] = $this->requestData->temple_id;
    ...
    function view_receipt1_post(){
        ...
        if(!empty($receipt)){
            if($receipt['receipt_type'] == "Pooja"){
				...
				$receiptArray['receiptDetails'] = $this->api_model->get_receipt_details($this->requestData->receipt_id);
				foreach($receiptArray['receiptDetails'] as $row){
					$receiptArray['receiptDetails']['scheduled_date']= date('d-m-Y',strtotime($row->date));
				}
				...
				$detailArray['address'] = $firstPostalData['address'];
				$detailArray['phone'] = $firstPostalData['phone'];
    ...
    function view_receipt_post(){
        ...
        if(!empty($receipt)){
            if($receipt['receipt_type'] == "Pooja"){
                if($receipt['pooja_type'] == "Normal"){
					$receiptDetails = $this->db->select('*,DATE_FORMAT(date, "%d-%m-%Y") as date')->where('receipt_id',$this->requestData->receipt_id)->get('opt_counter_receipt_details')->result();
                    ...
                    $date1 = $this->api_model->scheduled_pooja_date_new1($this->requestData->receipt_id,"asc",$this->requestData->language);
                    $date2 = $this->api_model->scheduled_pooja_date_new1($this->requestData->receipt_id,"desc",$this->requestData->language);
                    ...
                    $receiptDetails = $this->db->select('*,DATE_FORMAT(date, "%d-%m-%Y") as date')->where('receipt_id',$this->requestData->receipt_id)->get('opt_counter_receipt_details')->row_array();
                    $receiptCount = $this->db->select('*')->where('receipt_id',$this->requestData->receipt_id)->get('opt_counter_receipt_details')->num_rows();
					if(empty($receiptDetails)){
						$receiptDetails = $this->db->select('*,DATE_FORMAT(date, "%d-%m-%Y") as date')->where('receipt_id',$this->requestData->receipt_id)->get('receipt_details')->row_array();
				...
				$receiptArray['receiptDetails'] = $this->api_model->get_receipt_details($this->requestData->receipt_id);
				foreach($receiptArray['receiptDetails'] as $key => $row){
					$receiptArray['receiptDetails'][$key]->scheduled_date = date('d-m-Y',strtotime($row->date));
				}
            }else if($receipt['receipt_type'] == "Postal"){
                $totalCount = count($this->api_model->get_receipt_details($this->requestData->receipt_id));
                $firstPostalData = $this->api_model->get_part_receipt_detail($this->requestData->receipt_id,'asc');
                ...
                if($receipt['receipt_type'] == "Pooja"){
                    if($receipt['pooja_type'] == "Prathima Aavahanam"){
                            ...
                            if($this->api_model->get_prethima_aavahana_availability($aavahanamBookingDetail['booked_date'],$receipt_id)){
                                ...
                                if($this->api_model->update_aavahanam_booking($aavahanamBookingDetail['id'],$updateAavahanamBooking)){
                                    ...
                                    foreach($receiptDetails as $key => $val){
                                        $scheduledMalayalamDate1 = $this->common_model->get_malayalam_date($val->date);
										$receiptDetails[$key]->mal_date = $scheduledMalayalamDate1['malyear'].",".$scheduledMalayalamDate1['malmonth'];
										if($val->pooja_master_id == 79 || $val->pooja_master_id == 204){
											$receiptDetails[$key]->malyalam_cal_status = 1;
										}else{
											$receiptDetails[$key]->malyalam_cal_status = 0;
										}
                                    }
                        ...
                        foreach($this->responseData['data']['receipt'] as $key => $row){
                            ...
                            if($row->receipt_type == "Pooja" && $row->pooja_type == "Prathima Samarppanam"){
								...
								$receiptDetails['occurence'] = $receiptCount;
								$receiptDetails['receipt_no'] = $row->receipt_no;
                                ...
                                if($receiptCount == 1){
                                    ...
                                    $receiptDetails['scheduled_date']= date('d-m-Y',strtotime($date2['date']))." - ".date('d-m-Y',strtotime($date1['date']));
                                    $scheduledMalayalamDate2 = $this->common_model->get_malayalam_date($date1['date']);
                                    $scheduledMalayalamDate1 = $this->common_model->get_malayalam_date($date2['date']);
                                    $receiptDetails['scheduled_date_malayalam']= $scheduledMalayalamDate1['malyear'].",".$scheduledMalayalamDate1['malmonth']." - ".$scheduledMalayalamDate2['malyear'].",".$scheduledMalayalamDate2['malmonth'];
									$this->responseData['data']['receipt'][$key]->scheduled_date = date('d-m-Y',strtotime($date2['date']))." - ".date('d-m-Y',strtotime($date1['date']));
                        ...
                        foreach($this->responseData['data']['receipt'] as $key => $row){
                            ...
                            if($row->receipt_type == "Pooja" && $row->pooja_type == "Normal"){
                                ...
                                if(empty($receiptDetails))
                                    $receiptDetails = $this->db->select('*,DATE_FORMAT(date, "%d-%m-%Y") as date')->where('receipt_id',$row->id)->get('opt_counter_receipt_details')->result();
                                $this->responseData['data']['receipt'][$key]->details = $receiptDetails;
                    ...
                    if(!empty($annadhanamBookingDetail)){
                        ...
                        if($this->api_model->update_annadhanam_booking($annadhanamBookingDetail['id'],$updateAavahanamBooking)){
							...
							$this->responseData['data']['receipt']['scheduled_date'] = date('d-m-Y',strtotime($annadhanamBookingDetail['booked_date']));
                            $totalAmount = number_format((float)$this->responseData['data']['receipt']['receipt_amount'], 2, '.', '');
                            $this->responseData['data']['totalAmount'] = $totalAmount;
                            $this->responseData['data']['com_rece_id'] = $receipt_id;
