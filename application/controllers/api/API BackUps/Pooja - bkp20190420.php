...
class Pooja extends REST_Controller {
    ...
    function get_daily_nivedya_list_post(){
        ...
        $additional_booked = $this->Daily_list_model->get_additional_booked_prasadam_list($date,$this->requestData->temple_id,$this->requestData->language);
        foreach($additional_booked as $key=>$row){
            $additional_booked[$key]->defined_quantity = $row->defined_quantity*$row->quantity;
        }
        //$additional_list = $this->Daily_list_model->get_additional_nivedya_list($date,$this->templeId,$this->languageId);
        $this->responseData['data']['booked_pooja_list'] = array_merge($booked_pooja_list,$additional_booked);
        $this->response($this->responseData);
    }
    ...
    function aavahanam_final_payment_post(){
        ...
        if(!empty($aavahanamFinalPayData)){
            if($aavahanamFinalPayData['status'] == "BOOKED"){
                ...
                /**Postal Charge Start */
                $postalRate = 0;
                if($advanceReceiptDetails['postal_check'] == 1 && !empty($poojaDates)){
                    $receiptMainData = array();
                    $postalRate = $this->common_model->get_postal_rate();
                    $receiptMainData['receipt_type'] = "Postal";
                    $receiptMainData['pooja_type'] = "Prathima Aavahanam";
                    ...
                    foreach($poojaDates as $val){
                        ...
                        $postalReceiptDetailData[$xpostal]['rate'] = $postalRate['rate'];
                        $postalReceiptDetailData[$xpostal]['quantity'] = 1;
                        $postalReceiptDetailData[$xpostal]['amount'] = $postalRate['rate'];
                        $postalReceiptDetailData[$xpostal]['date'] = date('Y-m-d',strtotime($val->gregdate));
                        $postalReceiptDetailData[$xpostal]['phone'] = $this->requestData->phone;
                        $postalReceiptDetailData[$xpostal]['address'] = $this->requestData->address;
                    }
