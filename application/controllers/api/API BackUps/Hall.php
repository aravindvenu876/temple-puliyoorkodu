        ...
        if(date('Y-m-d',strtotime($this->requestData->from_date)) <= date('Y-m-d')){
            ...
            if(date('Y-m-d',strtotime($this->requestData->from_date)) > date('Y-m-d',strtotime($this->requestData->to_date))){
                ...
                if($checkBlockStatus == 0){ 
                    // $this->response($this->api_model->check_hall_availability1($this->requestData));
                    if($this->api_model->check_hall_availability1($this->requestData)){
                        ...
                        if($balance > 0){
                            ...
                            if(!empty($receipt_id)){
                                ...
                                //     $accountEntryMain['sub_type2'] = "Cash";
                                // }
                                // $accountEntryMain['head'] = $this->requestData->hall_id;
    ...
    function hall_final_payment_post(){
        ...
        if(!empty($hallFinalPayData)){
            if($hallFinalPayData['status'] == "BOOKED"){
                ...
                // $receiptMainData['receipt_no'] = $receiptNumber;
                $receiptMainData['receipt_type'] = "Hall";
                if($amountFlag == 1){
                ...
                if(!empty($receipt_id)){
                    ...
                    // $accountEntryMain['date'] = date('Y-m-d');
                    // $accountEntryMain['voucher_no'] = $receipt_id;
                    // $accountEntryMain['amount'] = $hallFinalPayData['advance_paid'] + $this->requestData->amount;
                    // $accountEntryMain['description'] = "";
                    // $accountEntryMain['accountType'] = "Kalyanamandapam Receipts";
                    // $accountEntryMain['sub_type3'] = "Hall Advance";
                    // $accountEntryMain['amount2'] = $this->requestData->amount;
                    // $accountEntryMain['amount3'] = $hallFinalPayData['advance_paid'];
                    // $this->accounting_entries->accountingEntry($accountEntryMain);
                    /**Accounting Entry End */
                }else{
                    $this->responseData['status'] = FALSE;
