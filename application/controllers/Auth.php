...
class Auth extends CI_Controller {
        ...
        if ($this->tank_auth->is_logged_in()) {         // logged in
            ...
            if ($this->form_validation->run()) {        // validation ok
                if ($this->tank_auth->login(
                                $this->form_validation->set_value('login'), $this->form_validation->set_value('password'), $this->form_validation->set_value('role'), 1, $data['login_by_username'], $data['login_by_email'])) {        // success
                    redirect('welcome');
                } else {
     ...
     * @return void
     */
    function staff_portal_login() {
        if ($this->tank_auth->is_logged_in()) {         // logged in
            redirect('Welcome/portal');
        } else {
            $data['login_by_username'] = ($this->config->item('login_by_username', 'tank_auth') AND
                    $this->config->item('use_username', 'tank_auth'));
            $data['login_by_email'] = $this->config->item('login_by_email', 'tank_auth');
            ...
            if ($this->form_validation->run()) {        // validation ok
                if ($this->tank_auth->login(
                                $this->form_validation->set_value('login'), $this->form_validation->set_value('password'), 1, $data['login_by_username'], $data['login_by_email'])) {        // success
                    redirect('welcome/portal');
                } else {
                    $errors = $this->tank_auth->get_error_message();
                    if (isset($errors['banned'])) {        // banned user
            ...
            $this->load->view('auth/staff_portal', $data);
        }
    }
        ...
        if (!$this->tank_auth->is_logged_in()) {        // not logged in or not activated
            ...
            if ($this->form_validation->run()) {        // validation ok
                if ($this->tank_auth->change_password(
                    ...
                    $this->session->set_flashdata(array('msg' => 'Password Changed Successfully', 'status' => '1'));
                    redirect('dashboard');
                } else {              // fail
                    $errors = $this->tank_auth->get_error_message();
                    $er = "";
                    ...
                    $this->session->set_flashdata(array('msg' => $er, 'status' => '0'));
                    redirect('dashboard');
                }
            }
    ...
    function switch_language($id){
        ...
        $this->session->set_userdata(array('language' => $id));
        $this->common_functions->set_language();
        redirect($url);
    }

    function switch_temple($id){
        $url= $_SERVER['HTTP_REFERER'];
        $this->session->set_userdata(array('temple' => $id));
        redirect($url);
    }

    function switch_database($id){
        $url= $_SERVER['HTTP_REFERER'];
        $this->session->set_userdata(array('database' => $id));
        if($id == 'default'){
            $this->session->set_userdata(array('fin_year' => '2022 September To 2023 March'));
        } else if($id == '20192022'){
            $this->session->set_userdata(array('fin_year' => '2019 April To 2022 July 18'));
        }
        redirect($url);
    }
    ...
    function _send_email($type, $email, &$data) {
        ...
        $this->email->from('IICT');
        $this->email->reply_to('no-reply@iictindia.org');
        $this->email->to($email);
