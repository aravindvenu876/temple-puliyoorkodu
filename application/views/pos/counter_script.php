...
<script  type="text/javascript">
    ...
    var action_url = $('#counters').attr('action_url');
    oTable = gridSFC('counters', action_url, aoColumnDefs);
    detail('<?php echo base_url() ?>service/POS_data/counter_edit', function(data) {
        detail_edit(data);
    });
    function detail_edit(data) { //////////////////////////// Form and Grid ///////////////////////////////////////////////
        $(".plus_btn").trigger('click');
        $("#form_title_h2").html("<?php echo $this->lang->line('update_counter'); ?>");
        $(".saveButton").text("<?php echo $this->lang->line('update'); ?>");
        $("form.add-edit").attr('action', "<?php echo base_url() ?>service/POS_data/counter_update");
        $('#counter_no').val(data.editData.counter_no);
        $("#data_grid").val(oTable.attr("id"));
        $("#selected_id").val((data.editData.id));
    }
    $(".plus_btn").click(function() {
        $("#form_title_h2").html("<?php echo $this->lang->line('add_counter'); ?>");
        $(".saveButton").text("<?php echo $this->lang->line('save'); ?>");
        $("form.add-edit").attr('action', "<?php echo base_url() ?>service/POS_data/counter_add");
        clear_form();
    });
</script>