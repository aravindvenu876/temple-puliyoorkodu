...
<script type="text/javascript">
    ...
    var action_url = $('#view_sb_to_fd').attr('action_url');
    oTable = gridSFC('view_sb_to_fd', action_url, aoColumnDefs);
    function get_scheduled_pooja_list(){
        $("#view_sb_to_fd").dataTable().fnDraw();
    }
    ...
    $.ajax({
        ...
        success: function (data) {
			var string = '';
			var string1 = '<option value="">Select Bank</option>';
    ...
    $.ajax({
        url: '<?php echo base_url() ?>service/Rest_shared/get_zero_fd_accounts_drop_down',
        type: 'GET',
        success: function (data) {
            var string = '<option value="">Select FD Account</option>';
    ...
    $(".plus_btn").click(function() {
        $("#form_title_h2").html("<?php echo $this->lang->line('add_sb_to_fd_transfer'); ?>");
        $(".saveButton").text("<?php echo $this->lang->line('save'); ?>");
        $("form.add-edit").attr('action', "<?php echo base_url() ?>service/Bank_data/sb_to_fd_transfer_add");
        clear_form();
    });
</script>
